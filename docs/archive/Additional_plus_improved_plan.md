...
# Jibon Niye Khela ΓÇö Phase 7ΓÇô11 Expansion Plan
...
ΓÇö it builds on top of them. Read all four before starting any phase here.
Each phase below has its own **Gate** (pass/fail test checklist) in the
same style as `TESTING.md` ΓÇö a phase is not done until its Gate passes
...
that as **emotional/functional parity** (the same *kind* of moment ΓÇö
a baby-crying beat at birth, a triumphant sting on a big win, dating and
marriage mechanics, huge event variety) ΓÇö never as literally copying
...
properly-licensed assets and fictional characters, per `AGENT.md` ┬º9 and
`DESIGN.md` ┬º11. If the agent is ever unsure whether something crosses
that line, it must stop and ask rather than "just make it close enough."

---

## Phase 7 ΓÇö Emotional Audio System (2 BGM tracks + reactive human SFX)
...
death), and exactly **two** background music tracks total ΓÇö one for the
"early life" age span, one for the "late life" age span ΓÇö instead of a
larger rotating music set.

### Tasks
1. **Reduce BGM to exactly 2 tracks**, per the user's explicit direction:
   - `bgm_early_life` ΓÇö plays from character birth through roughly the
     end of the Teen life stage (`DESIGN.md` ┬º4: ages 0ΓÇô17). Should read
     as lighter/warmer in tone.
   - `bgm_late_life` ΓÇö plays from Young Adult onward (ages 18+) through
   ...
   - Remove any additional per-life-stage music tracks that may already
     exist from the earlier Milestone 3 pass ΓÇö this is a deliberate
...
2. **Add a small set of reactive human-toned SFX**, functionally
   ...
   - `sfx_birth` ΓÇö plays once when a new life starts (e.g., a short,
     original baby-cry-style vocalization, either a licensed CC0 clip or
     a simple synthesized/foley substitute ΓÇö do not source or rip
     BitLife's actual audio file).
   - `sfx_good_event` ΓÇö a short, warm human-toned affirmation cue (an
     "oh nice!"-feeling sound ΓÇö can be a vocalization, a chime with a
   ...
   - `sfx_bad_event` ΓÇö a short, human-toned dismay cue ("oh no"-feeling)
     for clearly negative outcomes.
   - `sfx_death` ΓÇö a distinct, weightier cue for the death/end-of-life
     moment ΓÇö somber, not comedic, and not reused for any other event.
   - Map these to the existing event `tone` field (`good | bad | neutral
     | funny`, per `DESIGN.md` ┬º10) so the correct cue fires automatically
     ΓÇö `funny`-toned events can use either the good or a distinct
   ...
   - All new SFX must go through the existing `SoundManager` (`/lib/audio`)
     ...
     Milestone 3 ΓÇö no new bypass path.
3. **Source/produce these assets legitimately**: CC0 sources (freesound.org
   ...
   `/public/audio/CREDITS.md` per `AGENT.md` ┬º9 ΓÇö this file must grow to
...
### Gate 7 ΓÇö must pass before Phase 8
- [ ] Confirm via the audio manifest / bundle that exactly 2 BGM files are
      referenced project-wide ΓÇö grep for any leftover references to
...
- [ ] Trigger one `good`, one `bad`, one `funny`, and the birth and death
      moments, and confirm the correct SFX fires for each ΓÇö spy/mock the
...
- [ ] `/public/audio/CREDITS.md` lists every new asset added this phase
      with a real source/license ΓÇö spot-check 3 entries against actual
      files on disk.
- [ ] Confirm no BitLife-sourced or otherwise unlicensed audio file was
      added ΓÇö state explicitly where each new asset came from.

---

## Phase 8 ΓÇö Responsive Layout Overhaul (de-centered, full-bleed UI)
...
middle, and the player should never need to scroll horizontally ΓÇö only
...
1. **Define responsive breakpoints and a real layout, not a single
   ...
   - **Mobile (< 768px):** current single-column stack is acceptable
     here ΓÇö this is the one width where centered-stack genuinely is the
     right pattern ΓÇö but tighten side margins so content uses more of
     the available width (per `UI-DESIGN.md` ┬º1.3's 8px grid, not
     arbitrary large auto-margins).
   - **Tablet (768ΓÇô1279px):** two-column layout ΓÇö character card + stat
     bars + action buttons in a persistent left/side column, event
     history (Chronicle/Life Log) in the larger remaining column.
   - **Desktop (ΓëÑ 1280px):** three-region layout ΓÇö e.g., character
     ...
     leaving dead space ΓÇö never ship a wide viewport with large empty
     margins on both sides.
2. **Make the event log its own scroll container** (`overflow-y: auto`
   on that region specifically) rather than the whole page scrolling ΓÇö
...
3. **No horizontal scrolling at any supported width**, including the
   Life Actions tab row and Family Tree zoom controls ΓÇö verify tab rows
...
4. **Audit every existing screen** (Landing, Character Hub, Life Actions
   panel, Family Tree panel, Settings) against this new layout system ΓÇö
   this is a genuine restructure of `UI-DESIGN.md` ┬º2's screens, so
...
5. Keep every existing design token from `UI-DESIGN.md` ┬º1 (colors,
   radius, typography) ΓÇö this phase changes **layout/composition**, not
   the visual language established in the earlier reskin.

### Gate 8 ΓÇö must pass before Phase 9
- [ ] Screenshot the Character Hub at 375px (mobile), 900px (tablet), and
      ...
      width-appropriate layout ΓÇö not the same centered column scaled up
...
- [ ] Confirm the event/Chronicle log has its own internal scroll and
      ...
      reachable without scrolling past a long history ΓÇö test with a
      character that has aged up at least 15 times so there's real
      history to scroll through.
- [ ] Confirm no dead/empty large-margin space exists at desktop width ΓÇö
...
- [ ] `UI-DESIGN.md` ┬º2 has been updated to reflect the new responsive
      layout ΓÇö paste a diff or the updated section.

---

## Phase 9 ΓÇö Content Depth, Randomness, Custom Life, and Romance Systems
...
boyfriend, marriage, cheating, breakups) that currently don't exist ΓÇö
...
**9.1 ΓÇö Anti-repetition & variety engine (engine-level, do first)**
- Add a **recent-event cooldown**: track the last N (e.g., 15ΓÇô20) event
...
- Add a **templated-text layer**: for event copy, support randomized
  filler slots (e.g., a bank of 4ΓÇô6 alternate phrasings/synonyms per
  ...
  effort.
- Raise the content volume target from `DESIGN.md` ┬º10's 150ΓÇô250 events
  to **300ΓÇô400 unique event templates**, leaning the increase toward
  Teen/Young Adult/Adult stages where replay frequency and the new
  romance systems (below) create the most demand for variety.

**9.2 ΓÇö Custom Life creator (new feature, needs its own scoped design)**
- Add an optional "Custom Life" entry point alongside the existing
  ...
  era flavor, a starting family wealth tier (poor/middle/wealthy ΓÇö this
  should bias starting money and a few early stat rolls, not hard-lock
  outcomes), and optionally 1ΓÇô2 starting traits from a curated list.
  This mirrors BitLife's custom-start concept but must stay entirely
  free (no paid "God Mode" tier ΓÇö the whole game is free per `AGENT.md`
  ┬º2).
- This touches the engine's character-creation function
  (`createCharacter`) ΓÇö extend it to accept optional overrides rather
...
**9.3 ΓÇö Romance & relationship systems (new systems, largest task here)**
- **Dating:** starting in the Teen stage (13+, strictly non-sexual ΓÇö
  school-dance/crush-level content only, per `DESIGN.md` ┬º11), add a
  ...
  generation system used for family members ΓÇö confirm this reuses the
  fixed name-generation logic from `UI-DESIGN.md` ┬º0.5 point 2 and does
...
- **"Celebrity-type" dating pool:** implement as a distinct, clearly
  ...
  "Indie Filmmaker") with procedurally generated fictional names ΓÇö never
...
- **Relationship progression (18+ for anything beyond dating):** ask out
  ΓåÆ date ΓåÆ make it official (girlfriend/boyfriend) ΓåÆ move in ΓåÆ propose ΓåÆ
  marry ΓåÆ optionally have kids (already scoped in `DESIGN.md` ┬º5.3) ΓåÆ
...
- **Cheating branch:** allow the player to choose to cheat as a narrative
  ...
  risk, breakup/divorce events, reputation/karma impact) ΓÇö this is a
  consequence-driven mechanic, not a rewarded/glorified one; keep this
  consistent with how `DESIGN.md` ┬º11 wants crime handled (real
...
- **Tone ("spicier," funnier):** widen the humor/absurdity range of
  ...
  category in `DESIGN.md` ┬º10) and allow flirtatious/suggestive-but-not-
  explicit romantic flavor text for adult (18+) relationship events ΓÇö
  ...
  the hard rule in `DESIGN.md` ┬º11. This is a firm line the agent must
...
- [ ] **Templated-text variety test:** trigger the same event template
      ...
      grammatically ΓÇö paste example outputs.
...
- [ ] **Romance system walkthrough:** demonstrate (test or described
      manual playthrough with evidence) a full romance arc ΓÇö date ΓåÆ
      relationship ΓåÆ marriage ΓåÆ an optional cheating branch ΓåÆ divorce ΓÇö
...
- [ ] **No real people:** grep all new romance/dating content for any
      real celebrity or public-figure name ΓÇö must return zero matches.
...
- [ ] **Age-gating audit:** review every new romance-related event and
      ...
      ΓÇö list how many events were reviewed.
- [ ] Content volume check re-run per `TESTING.md` Gate 5 against the new
      300ΓÇô400 target, confirming no life stage has zero eligible events.

---

## Phase 10 ΓÇö Keyboard Shortcuts & Pause System
...
### Tasks
1. **Pause system:** if no pause/paused-state currently exists, add one ΓÇö
   ...
   Landing). This must not corrupt in-progress state ΓÇö pausing mid-Age-Up
...
2. **Keyboard shortcut set** (document these in a discoverable in-app
   ...
   - `Esc` ΓÇö open/close the pause menu (or close the topmost open
     modal/panel if one is open ΓÇö Esc should close panels before it
     opens the pause menu, so it behaves predictably like other apps).
   - `Enter` / `Space` ΓÇö confirm the focused button/primary action
     (e.g., advance an event, trigger Age Up when it's focused).
   - `1`ΓÇô`4` ΓÇö select the corresponding numbered choice on an active
     event card, when choices are present.
   - Arrow keys / `Tab` ΓÇö navigate focus between interactive elements in
   ...
   - `?` ΓÇö open a shortcuts-reference overlay.
...
### Gate 10 ΓÇö must pass before Phase 11
- [ ] Complete one full life (birth to death) using only the keyboard ΓÇö
      no mouse ΓÇö and document the exact key sequence used.
...
- [ ] Confirm pausing mid-event and resuming does not lose or duplicate
      the in-progress event/choice state ΓÇö test this explicitly.
...
- [ ] Confirm shortcuts do not fire while a text input is focused (test
      by focusing the Custom Life name field and pressing `1`ΓÇô`4`/`Esc`,
      confirming normal typing behavior instead of a shortcut firing).
- [ ] `?` shortcuts overlay lists every shortcut above accurately.

---

## Phase 11 ΓÇö Public Landing / Marketing Page

### Objective
Build a proper pre-game marketing/landing experience ΓÇö this is distinct
from the in-game "New Life" screen from `UI-DESIGN.md` ┬º2.1. This is the
...
1. **Hero section (above the fold):**
   ...
   - Primary CTA: a large, unmissable "Play Free in Browser" button that
     takes the visitor straight into the existing New Life flow ΓÇö no
   ...
   - Atmospheric visual: a stylized screenshot or short looping clip of
     the actual reskinned UI (per `UI-DESIGN.md`) ΓÇö use real product
...
2. **Core features & gameplay section:**
   - A short looping GIF/video (a few seconds) showing an event card
     resolving and a stat bar reacting ΓÇö real captured footage of the
     actual game, not staged/faked UI.
   - Exactly 3 short, punchy bullet points on what makes the game
     distinct (e.g., "Every life is different ΓÇö huge event variety and a
     custom-life creator," "Reactive stats, sounds, and moments ΓÇö not
...
3. **Clean technical execution:**
   - Keep this page's own bundle lean and separate from the game's main
     bundle ΓÇö the landing page should not eagerly load Lottie/Howler/
     the full game engine; those load only once the visitor clicks Play.
   - Fully responsive at mobile widths ΓÇö this page gets the same
   ...
   - Target a fast Lighthouse Performance score for this specific page
     (this is the first-impression page ΓÇö hold it to at least the same
     bar as `TESTING.md` Gate 6's in-game target, ideally higher since
     it's simpler and has less to load).

### Gate 11 ΓÇö must pass before calling the whole expansion done
- [ ] Landing page Lighthouse scores (Performance/Accessibility/Best
      Practices) meet or exceed the Gate 6 in-game targets ΓÇö paste actual
      scores.
- [ ] Confirm the landing page's initial JS payload does not include the
      game engine, Howler, or Lottie ΓÇö those must only load after the
      Play CTA is clicked (network tab evidence).
- [ ] Screenshot the landing page at 375px, 900px, and 1440px widths ΓÇö
...
- [ ] Confirm the demo GIF/video is real captured footage of the shipped
      app (not a mockup) ΓÇö describe how it was captured.
...
```
Phase 7  (Audio)          ΓåÆ Gate 7  ΓåÆ 
Phase 8  (Layout)         ΓåÆ Gate 8  ΓåÆ
Phase 9  (Content/Romance)ΓåÆ Gate 9  ΓåÆ
Phase 10 (Keyboard/Pause) ΓåÆ Gate 10 ΓåÆ
Phase 11 (Landing page)   ΓåÆ Gate 11 ΓåÆ Full expansion complete
```
...
full ΓÇö the "did we actually build what we wanted" holistic pass ΓÇö since
