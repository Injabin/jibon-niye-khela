# DESIGN.md ΓÇö Game Design Document

Project name: **Jibon Niye Khela** ("Playing with Life") ΓÇö must not use
...
outcomes ΓÇö then dies, gets a life summary, and starts again.

Design pillars:
1. **Every tap has weight** ΓÇö even small choices nudge stats or spawn
   follow-up events later in life.
2. **Read less, feel more** ΓÇö modern UI shows change through motion/sound,
   not just number deltas in text.
3. **Replayability over completeness** ΓÇö breadth of silly/serious event
   variety matters more than depth of any single system at launch.
4. **Free forever core loop** ΓÇö no paywalls; cosmetic-only future extras.

## 2. Core loop

```
New Character ΓåÆ Age Up (yearly tick) ΓåÆ Random Events resolve/choices made
   ΓåÆ Age-gated Activities available (school, job, relationships, crime,
     health, assets) ΓåÆ repeat Age Up ΓåÆ Death (event, old age, or player
     choice like suicide-risk handling ΓÇö see ┬º11 sensitivity notes)
   ΓåÆ Life Summary screen ΓåÆ New Character (same family tree optionally) or
     Start Fresh
```

Two primary interaction modes:
- **Passive tick:** press "Age Up" ΓåÆ stat deltas animate, 0ΓÇô3 random
...
Character {
  ...
  stats: { health, happiness, smarts, looks }   // 0ΓÇô100, decay/grow yearly
  money: number
  age: number
  alive: boolean
  causeOfDeath?: string
  traits: string[]                // e.g. "Class Clown", "Prodigy" ΓÇö unlock event variants
...
Stats (0ΓÇô100, visualized as animated radial/bars):
- **Health** ΓÇö decays with age, drugs, accidents; restore via
  gym/hospital/therapy; 0 ΓåÆ death.
- **Happiness** ΓÇö swings with events/relationships/activities; chronic
  low happiness unlocks negative-spiral events (see ┬º11).
- **Smarts** ΓÇö driven by school/reading/hobbies; gates education & jobs.
- **Looks** ΓÇö driven by genetics (rolled at birth) + gym/plastic surgery;
...
| Stage | Ages | Unlocks |
|---|---|---|
| Infant | 0ΓÇô2 | Parent-controlled mini events only |
| Toddler | 3ΓÇô5 | First personality/trait rolls, preschool |
| Child | 6ΓÇô12 | Elementary school, allowance, first friendships, pets |
| Teen | 13ΓÇô17 | High school, part-time jobs, dating, first car, curfew/crime intro |
| Young Adult | 18ΓÇô25 | University/vocational, first full job, moving out, marriage, military |
| Adult | 26ΓÇô64 | Career growth, family, assets, business ownership |
...
roll 0ΓÇô3 events from the weighted pool filtered by age/stage/traits/flags,
...
Copy/events must be original per AGENT.md ┬º9.

### 5.1 Education
Preschool ΓåÆ Elementary ΓåÆ Middle ΓåÆ High School (with GPA, popularity,
clubs, sports, bullying/being-bullied branches, expulsion risk) ΓåÆ
University/College (majors gated by Smarts, tuition vs. loans, Greek life,
dropout risk) ΓåÆ Grad school / vocational / trade school.
...
Fame (streamer, actor, musician ΓÇö procedurally named, not real people),
Politics (fictional offices only), Sports.

### 5.3 Relationships
Family tree (parents, siblings, grandparents, spouse, children, pets)
rendered as an interactive node graph (see ┬º7). Relationship meter per
person (0ΓÇô100) affected by "Activities" menu actions (talk, compliment,
...
Read a book (smarts), Hobbies (music, art, sports ΓÇö can snowball into
careers/fame), Social media (fame/happiness with risk of controversy
events), Pets (adopt/train/lose), Travel (vacations ΓÇö happiness burst +
...
### 5.6 Crime & law
Petty crime ΓåÆ grand theft ΓåÆ organized crime ladder with escalating
...
hospital visits, addiction mechanics (see ┬º11 for sensitivity handling),
...
can pick/write) ΓåÆ offer to play as an heir/child (legacy mode) or start
new.
...
"Straight-A student". Purely for replay incentive ΓÇö no gameplay power.

## 6. What makes this feel "more interactive & premium" than the genre norm

This is the differentiator the user asked for ΓÇö treat it as core scope,
not polish-later:
...
2. **A living character portrait/avatar.** A layered 2D avatar with Lottie
   expression overlays (see ┬º7) that visibly ages, changes expression, and
   gets small visual tells (sweat drop on a risky choice, sparkle on a
   win) ΓÇö this is how "every reaction feels alive," kept lightweight.
3. **Stat bars that fight back.** When a choice tanks Happiness, the bar
   doesn't just shrink ΓÇö it shakes, flashes red, and the number ticks down
...
4. **Full-screen "moment" beats.** For milestone events (graduation,
   ...
   music cue briefly takes over full-screen, then returns to the menu ΓÇö
...
5. **Sound as feedback, not decoration.** Every button press, stat change,
   ...
   improved_plan Phase 7) ΓÇö `early_life` (ages 0ΓÇô17, lighter/warmer) and
   `late_life` (18+, more mature/weightier) ΓÇö which **crossfade** at the age
   ...
   ┬º10) so the right reaction fires automatically.
6. **Haptics on mobile** (`navigator.vibrate`) mirroring the SFX for key
   negative/positive beats, where supported.

## 7. The avatar & "moment" layer ΓÇö scope discipline

**Decision: 2D layered sprite + Lottie, not real-time 3D.** This is
locked for v1 ΓÇö it's faster to build, far lighter on bundle size/battery
...
stretch goal, not part of the MVP scope ΓÇö do not add the `three`/
...
- **Avatar:** a layered 2D character built from swappable PNG/SVG parts
  ...
  per DESIGN.md ┬º10). Body/outfit swaps at each life stage transition
  (DESIGN.md ┬º4).
...
- **Milestone moments:** short, reusable Lottie "sting" animations
  ...
  by event **category**, not hand-built per unique event ΓÇö build 8ΓÇô10
  reusable stingers, not hundreds. Combine with a screen-shake/flash
  Framer Motion effect and the matching SFX (DESIGN.md ┬º6 point 5) for
  the "premium moment" beat.
- **Always provide a reduced-motion fallback** (Settings ΓåÆ "Reduce
...
- **Source/build Lottie assets** as CC0/original only (e.g., built in
  ...
  simple after-effects-free JSON animations authored directly) ΓÇö log
  licenses in `/public/animations/CREDITS.md` per AGENT.md ┬º9.
...
- **Event resolution pattern:** card slides/pops in ΓåÆ player picks an
  option (1ΓÇô4 choices, sometimes just "Continue") ΓåÆ outcome text +
  stat deltas animate in ΓåÆ card dismisses ΓåÆ return to hub.
...
- Single save = one "world" containing the active character plus any
  ...
  (MVP) ΓåÆ `IndexedDB` once saves grow (multiple past lives, big trees).
- Save includes a `schemaVersion`; `/lib/save` must ship a migration
  function for every schema bump ΓÇö never silently break old saves.
...
- Crime, death, and health decline are depicted, but never with graphic
  gore descriptions ΓÇö keep it stylized/cartoonish, not graphic.
- Suicide/self-harm are **not** implemented as a player-selectable
  ...
  line only ΓÇö never a chosen action with a mechanical reward.
- No sexual content involving minor-aged characters, ever, at any life
  stage ΓÇö romance/relationship content for characters under 18 stays
...
- Does the game still feel complete with 3D/sound fully disabled? (It
  must ΓÇö that's the accessibility bar.)
