# TESTING.md ΓÇö Acceptance Criteria & Test Gates
...
fails, the milestone is NOT done ΓÇö fix it, re-run, then proceed.
...
## Gate 0 ΓÇö after Milestone 0 (Scaffold)
...
- [ ] `git log` shows an initial commit history that isn't a single mega-commit ΓÇö at least a repo-init commit, a `.gitignore` commit or inclusion, and a tooling-setup commit, all with Conventional Commit messages.
- [ ] The live Vercel URL returns HTTP 200 and renders something (not a 404/500).
- [ ] The exact folder structure from `AGENT.md` ┬º4 exists ΓÇö list the actual `tree` output, don't just claim it.
- [ ] `theme.ts`/Tailwind config contains real token values (color roles, spacing scale, motion durations) ΓÇö no components should yet reference raw hex codes because none exist yet.
- [ ] `.gitignore` correctly excludes `node_modules`, `.next`, `.env*` ΓÇö confirm by running `git status` after a `npm install` and a build; neither should appear as untracked-but-should-be-ignored.
- [ ] README.md exists with setup instructions that a stranger could follow from a clean clone.

**Fail conditions that block moving to Milestone 1:** any command above exits non-zero; folder structure doesn't match AGENT.md ┬º4; repo/README missing.

---

## Gate 1 ΓÇö after Milestone 1 (Engine core, no UI)

This is the most important gate in the whole project ΓÇö if the simulation
...
1. **RNG determinism**
   - Same seed ΓåÆ identical sequence of outputs, across multiple calls and across a fresh process run.
   - Different seeds ΓåÆ sequences must differ (basic sanity, not a formal randomness proof).
...
2. **Stat clamping**
   ...
   - Test all four stats (health, happiness, smarts, looks) individually ΓÇö don't just test one and assume the others share the code path correctly.

3. **Death trigger correctness**
   - Health reaching exactly 0 sets `alive: false` and a `causeOfDeath`.
   - A character already dead does not process further `ageUp` calls (or explicitly rejects/no-ops ΓÇö pick one behavior and test it, don't leave it undefined).
...
5. **Event filtering correctness**
   - An event with `minAge: 13, maxAge: 17` must never fire for a character aged 12 or 18 ΓÇö assert this directly with boundary values (12, 13, 17, 18), not just a "seems age-appropriate" spot check.
...
6. **Weighted selection sanity check**
   - Run event selection 1,000 times with a fixed seed set (or seed range) and assert that a higher-weighted event fires meaningfully more often than a lower-weighted one ΓÇö not exact ratios, but a clear statistical direction, so weighting isn't silently a no-op.

7. **Full-life simulation stress test**
   - Simulate 200 full lives (birth ΓåÆ death) using 200 different seeds, with no UI involved, headless.
...
8. **Save schema round-trip at the engine level**
   - `serialize(character)` ΓåÆ `deserialize(...)` must produce a character deeply equal to the original, for at least: a freshly created character, a character mid-life with relationships/assets populated, and a dead character with full history.
...
## Gate 2 ΓÇö after Milestone 2 (Store, save, barebones UI)
...
- **Full loop smoke test (Playwright), no UI polish required yet, but must exist:** create a character ΓåÆ age up until an event with choices fires ΓåÆ select a choice ΓåÆ assert stats changed accordingly ΓåÆ continue aging until death ΓåÆ assert a life summary (even placeholder) is shown.

Manual checks the agent must explicitly confirm, not assume:
- [ ] Choosing different options on the same event produces genuinely different stat outcomes (prove this by aging up with two different seeded runs/choices and comparing results) ΓÇö this confirms choices aren't cosmetic.
...
## Gate 3 ΓÇö after Milestone 3 (Motion & sound pass)

Automated: typecheck/lint/test/build must still all pass ΓÇö a common regression here is animation/audio code accidentally breaking type-safety or introducing act-warnings in tests.
...
- [ ] **License file exists and is non-empty:** `/public/audio/CREDITS.md` lists every SFX/music asset used with its source and license ΓÇö spot-check at least 3 entries actually correspond to real files in `/public/audio`.
- [ ] **Visual regression sanity:** stat bar shake/glow and event card enter/exit animations are visible in a recorded Playwright trace or video, not just "implemented in code" ΓÇö attach or describe the evidence.
...
## Gate 4 ΓÇö after Milestone 4 (Avatar & Lottie moment layer)

- [ ] **Lazy-load verification:** inspect the network tab / bundle output and confirm `lottie-react` and its JSON assets are NOT present in the initial JS payload ΓÇö they must only load when an avatar/moment view is actually rendered. Paste the relevant bundle-analyzer or network evidence.
- [ ] **Expression mapping correctness:** for at least one `good`, one `bad`, and one `funny`-toned event, confirm (via test or described manual check with screenshots) the correct Lottie expression overlay fires ΓÇö not a generic/default one regardless of tone.
- [ ] **Milestone stinger coverage:** confirm at least 8 of the required 8ΓÇô10 reusable stinger animations exist and are wired to real event categories (list which categories map to which stinger file).
- [ ] **Reduced-motion fallback for avatar/Lottie specifically:** with the Settings toggle on, confirm Lottie playback is skipped and a static icon/fade is shown instead ΓÇö this is a distinct check from the Gate 3 Framer Motion check.
...
## Gate 5 ΓÇö after Milestone 5 (Content expansion)

- [ ] **Content volume check:** run a script that counts events in `/content` by category; confirm total is within the 150ΓÇô250 target from `DESIGN.md` ┬º10 and that no life stage (per `DESIGN.md` ┬º4) has zero eligible events ΓÇö a character must never be able to reach an age range with literally nothing to encounter.
- [ ] **Schema validation test:** every event object in `/content` validates against the typed event schema (id, text, minAge/maxAge, requiredFlags, statEffects, choices, weight, tags, tone) ΓÇö a single malformed event should fail this test loudly, not fail silently at runtime.
...
## Gate 6 ΓÇö after Milestone 6 (Polish, accessibility, launch prep)
...
## Gate UI-1 ΓÇö after the "Modern Martial" UI Overhaul (UI-DESIGN.md)
...
while "just doing UI" and silently breaking Gate 1ΓÇô2 guarantees. This gate
...
- [x] Re-run the full Gate 1 test suite (RNG determinism, stat clamping,
      ...
      ΓÇö that means engine logic was touched, which is out of scope for a
...
- [x] Confirm the stat mapping decision from `UI-DESIGN.md` ┬º0 was
      implemented as a **display-only relabel** ΓÇö write a test that
...
- [x] Grep the codebase for hardcoded hex colors outside `theme.ts`/
      Tailwind config in any touched component ΓÇö must return zero results.
...
- [x] Confirm border-radius is 4px on every card/button/modal/stat-bar
      container in the reskinned screens ΓÇö spot-check computed styles on
...
- [x] Build (or reuse) a single source-of-truth lookup table mapping
      stat/concept ΓåÆ color + icon, and write a test asserting every
...
### D. Layout correctness per UI-DESIGN.md ┬º2
- [x] Sticky header remains pinned during Chronicle Stream scroll ΓÇö
      verify with a Playwright scroll test, not a visual guess.
- [x] Sticky footer (Age Up + tab bar) remains pinned and does not
      overlap/clip the Chronicle Stream content at 360├ù640 and 360├ù740
      viewport sizes specifically (the smallest, highest-risk case called
      out in `UI-DESIGN.md` ┬º4) ΓÇö screenshot both sizes and confirm no
      clipped or unreachable content.
- [x] On Age Up, the Chronicle Stream auto-scrolls to the newest Year
      Card ΓÇö confirm this is instant (not smooth-scrolled) when
...
- [x] Interaction Overlay correctly dims background to the specified
      ...
      modal, not out to the dimmed background) ΓÇö this is both a visual
      and an accessibility requirement, test both.
- [x] Choice buttons meet the 48px minimum tap-height requirement ΓÇö
      measure computed height, don't eyeball it.

### E. Cross-theme regression
- [x] Toggle light/dark mode and confirm every token in ┬º1.1 switches
...
- Full Gate suites clean and untouched: `npx playwright test --workers=3`
  ΓåÆ **59 passed** (`*.spec.ts` in `tests/e2e/`, incl. `three-lives`,
  `keyboard-life`, `moments`, `legacy`, `save-flow`, `pwa`); `npm run test`
  ΓåÆ **207 passed** (Vitest, incl. `tests/ui/concepts.test.tsx` =
  `shards/statColors.ts` single source of truth for ┬º0 relabel +
...
- Gate A store-keys guard: `tests/ui/concepts.test.tsx` asserts store keeps
  ...
  "Martial Skill"/"Honor" (display-only relabel, per UI-DESIGN.md ┬º0).
- Gate B hardcoded-color grep: zero hex literals outside tokens; new AA
  ...
  4px everywhere incl. tabs (per UI-DESIGN.md ┬º1.3 "no pills"); 6 spot-check
...
- Gate C screenshots at `test-results/ui-theme/*.png`: `360x640-light.png`,
  `360x740-light.png`, `360x640-dark.png` cover the ┬º4 small-viewport
...
- Gate D: `ui-theme.spec.ts` pins sticky header + footer and verifies newest
  ...
  of the overlay); ΓëÑ48px tap-height measured on choice buttons.
- Gate E: `axe.spec.ts` (both color schemes ├ù hub landing / active life /
...
## Gate 7 ΓÇö after the "Emotional Audio" expansion (Additional_plus_improved_plan Phase 7)

Mirrored from `Additional_plus_improved_plan.md` Phase 7. The audio layer becomes
**two** background-music arcs (early 0ΓÇô17, late 18+) that **crossfade** at the age
boundary, plus a small curated set of **human-toned** reactive SFX cues. Every
┬º6.5 cue keeps working, no new third-party asset is added, and muting stays total.
...
- [ ] **Exactly two active BGM tracks, crossfaded:** no per-life-stage rotation, no
  ...
  (not a hard cut), fires once, in order (early ΓåÆ late).
- [ ] **Tone-accurate feedback cues fire through the real UI path:** a synthesized
  ...
  outcome its quirky sting, and the moment/death cues stay distinct ΓÇö verified by
...
- [ ] **No regression:** existing ┬º6.5 unit/motion/accessibility suites re-run green;
  engine determinism and save/Gate 1ΓÇô2 guarantees untouched.
...
## Final Gate ΓÇö "Did we actually build what we wanted?" (Full Launch QA)

Run this only once Gates 0ΓÇô6 have all individually passed. This is a
holistic regression pass against the *original intent*, not just each
milestone's narrow checklist ΓÇö the point is to catch things that pass
each gate individually but still add up to a game that doesn't feel right.

### A. Functional completeness vs. DESIGN.md
- [ ] Every system listed in `DESIGN.md` ┬º5 (Education, Career, Relationships, Activities, Assets, Crime, Health, Death/Legacy, Achievements) is reachable and produces a meaningfully different outcome depending on player choice ΓÇö walk through each system once end-to-end and describe what happened.
...
- [ ] With ALL motion/sound/animation disabled in Settings, the game is still fully completable and doesn't look broken (empty gaps, misaligned layout) ΓÇö this is the accessibility/perf floor and must be explicitly re-verified here, not assumed from earlier gates.
...
- [ ] Opening the app in a second tab with an existing save does not corrupt storage (describe what actually happens ΓÇö even "last write wins" is acceptable if it doesn't crash, but it must be stated, not left unknown).
...
- [ ] Total bundle size and Lighthouse scores at final state ΓÇö re-run, don't reuse Gate 6 numbers, since content/features grew since then.
...
4. Explicitly list any item it could NOT verify or had to skip, and why ΓÇö
...
section in full before answering ΓÇö a "yes" without that evidence is not
acceptable.
