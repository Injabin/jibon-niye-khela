# UI-DESIGN.md ΓÇö Visual System & Layout Specification
# Theme: "Modern Martial" (Claymore) ΓÇö Medieval Knight Reskin
...
the engine, the save schema, or the tech stack ΓÇö those remain governed by
...
implementation (see ┬º0).

Read `AGENT.md` ┬º8 (design-system discipline) before implementing anything
here ΓÇö all values below become the actual contents of `theme.ts`/Tailwind
config, not one-off inline styles.

---

## 0. Scope decision ΓÇö stat labels vs. stat mechanics (CONFIRM BEFORE BUILDING)

The reference spec lists four header stats: **Health, Honor, Martial Skill,
Happiness**. The existing engine (tested in Milestone 1, per `DESIGN.md` ┬º3)
...
|---|---|---|
| `health` | Health | ≡ƒ⌐╕ / heart-with-edge glyph |
| `happiness` | Happiness | ΓÇö (kept as-is, 4th position) |
| `smarts` | Martial Skill | ΓÜö∩╕Å |
| `looks` | Honor | ≡ƒ¢í∩╕Å |

This means: "Martial Skill" going up/down still runs through the exact same
`smarts` engine logic (book-smart events, school, etc.) ΓÇö only the label,
...
(driven by karma/reputation instead of genetics), **stop and say so** ΓÇö
...
gray on both canvas colors specifically ΓÇö these are the most likely to
...
that binding is used everywhere that concept appears ΓÇö header bars,
Chronicle Stream icons, event cards, Assets tab, Life Summary ΓÇö with zero
...
- **Font family:** `Inter` (primary), system-ui fallback stack. Load via
  `next/font` (self-hosted, zero external font-service dependency ΓÇö keeps
...
- **Border radius: 4px, everywhere.** Cards, buttons, input fields, stat
  ...
  corners anywhere in this theme ΓÇö this is the single most identity-
  defining rule of the reskin; treat any rounded-full element as a bug.
- **Elevation:** flat design ΓÇö use a 1px hairline border
  ...
  (┬º2.3) which gets one deliberate soft shadow to read as "elevated above
  the scrim."
...
- **Icons:** flat vector (prefer a small SVG icon set over emoji for
  production ΓÇö emoji renders inconsistently across OS/browsers and
...
```
Desktop (ΓëÑ 1280px): 3-Region Layout (3 cols : 6 cols : 3 cols)
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé LeftSidebar (Sticky)  Γöé Chronicle Stream (Main Center)  Γöé RightRail (Sticky)    Γöé
Γöé - Avatar & Identity   Γöé - Dedicated Internal Scroll     Γöé - Standing & Fame     Γöé
Γöé - 4 Core Stat Bars    Γöé   Container (overflow-y: auto)  Γöé - Active Relations    Γöé
Γöé - AGE (+1 YEAR)       Γöé - Year Card Dividers            Γöé - Possessions/Assets  Γöé
Γöé - Nav & Save Actions  Γöé - Tagged Event Dilemmas         Γöé - Quick Family Access Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ

Tablet (768pxΓÇô1279px): 2-Column Split (5 cols : 7 cols)
ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
Γöé LeftSidebar (Sticky)         Γöé Chronicle Stream (Main Body)                     Γöé
Γöé - Full Character Card        Γöé - Dedicated Internal Scroll (overflow-y: auto)   Γöé
Γöé - 4 Core Stat Bars           Γöé - Pinned year dividers                           Γöé
Γöé - AGE (+1 YEAR) & Nav        Γöé - Left sidebar remains fixed & fully reachable   Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
...
Γöé StickyHeader (Top ~15%, sticky top-0, compact summary + 4 stat bars)           Γöé
Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
Γöé Chronicle Stream (Scrollable body, pb-36, tight 8px-grid margins)               Γöé
Γö£ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöñ
Γöé ControlDeck (Fixed bottom-0, tactile AGE (+1 YEAR) button + 4-tab icon bar)    Γöé
ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
```
...
- Tech stack (`AGENT.md` ┬º3) ΓÇö unchanged.
- Engine logic, stat math, event-selection algorithm, save schema ΓÇö
  unchanged (see ┬º0's confirm-first rule).
- The 2D avatar + Lottie decision (`DESIGN.md` ┬º7) ΓÇö unchanged; the
...
- Sound design architecture (Howler/`SoundManager`) ΓÇö unchanged; SFX
...
## 4. Accessibility & responsiveness carry-overs (still apply, unchanged from AGENT.md ┬º7ΓÇô8)
...
- All of ┬º1ΓÇô2 must work at 360px width through desktop, sticky
  header+footer included ΓÇö verify header/footer don't collectively eat
  too much vertical space on short mobile viewports (test at 360├ù640 and
  360├ù740 specifically; the sticky header+footer combo is the highest
