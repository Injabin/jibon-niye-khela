# AGENT.md — Operating Rules for the Build Agent
...
| No server-side game logic requiring a paid DB | Game state lives client-side (see DESIGN.md §9) |
...
| Must be responsive: mobile (360px) → desktop (1920px+) | BitLife's audience is majority mobile |
| Must keep initial JS bundle lean | 3D/animation libs are heavy — lazy-load them |
| Never use the name "BitLife" anywhere in code, copy, or metadata | Trademark safety |

## 3. Tech stack (locked — do not swap without asking)
...
- **State management:** Zustand (small, serializable store — must be
...
- **Avatar/moment layer:** layered 2D sprite avatar + `lottie-web` (via
  ...
  (see DESIGN.md §7). **No React Three Fiber / WebGL for v1** — this is a
...
- **Persistence:** `localStorage` for MVP → upgrade path to `IndexedDB`
...
1. **Plan** — restate the feature in 3-5 bullets referencing the relevant
   DESIGN.md section before writing code.
2. **Engine first** — implement/extend pure logic in `/lib/engine` with
   unit tests. No UI yet.
3. **Store wiring** — expose the new state/actions via the zustand store.
4. **UI** — build the component, reuse existing design tokens (see §8),
   wire animation + sound reactions last.
5. **Verify** — run `npm run typecheck`, `npm run test`, `npm run lint`
   before considering the feature done.
6. **Self-review** — check against the "Definition of Done" in §10.
...
- First load JS ≤ ~200KB gzipped for the core loop (Lottie/audio libs
...
- Audio files: compressed (OGG/MP3 ≤ 64–96kbps for SFX), total initial
  audio payload ≤ ~1.5MB; stream/lazy-load ambient music.
...
- All colors, spacing, radii, shadows, and motion durations come from a
  single `theme.ts`/Tailwind config — no magic hex codes or ad-hoc `ms`
  values scattered in components.
- Motion has two tiers: **micro** (button press, stat tick — 100–200ms)
  and **moment** (life event resolves, death, big win — 400–900ms with
  the 3D/confetti/sound layer). Don't blur the two.
- Respect `prefers-reduced-motion` and provide a Settings toggle to
  disable 3D flourishes and/or sound entirely — required, not optional.
...
- No real celebrity names, no real politicians, no copyrighted characters,
  no depictions of real people — generate fictional NPCs only.
...
- Silently change the tech stack in §3.
